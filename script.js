...
const odometerImageGroup = document.getElementById('odometerImageGroup'); // ახალი: სურათის ატვირთვის ჯგუფი
...
function renderUIForRole(role) {
    ...
    if (role === 'courier') {
        ...
        odometerImageGroup.classList.add('hidden'); // კურიერისთვის დამალე სურათის ატვირთვის ველი
        checkCourierDayStatus(currentUserId); // შეამოწმე კურიერის დღის სტატუსი
        loadCourierData(currentUserId);
    } else if (role === 'admin') {
        adminPanel.classList.remove('hidden');
        odometerImageGroup.classList.remove('hidden'); // ადმინისთვის აჩვენე სურათის ატვირთვის ველი
        loadAllUsersData();
        loadAdminData();
    } else {
...
odometerForm.addEventListener('submit', async (e) => {
    ...
    // სურათის ატვირთვა მხოლოდ იმ შემთხვევაში, თუ ველი ხილვადია (ანუ ადმინია)
    if (odometerImageFile && !odometerImageGroup.classList.contains('hidden')) {
        try {
